...
class ClientReporitory{
    ...
    upTurn(){ // para que vaquero actualize el turno que se mostrara en la pagina de clientes y en el panel
        try {
            ...
            return { "newTurno": this.turnoNow, "clienteListo": clienteListo, "clienteActual": newClientActual, 'silla': this.silla }
    ...
    downTurn(){
        ...
        if (this.turnoNow > 1){
            try 
            {
                    ...
                    return {"newTurno": this.turnoNow, "clienteActual": cliente, "lastActual": lastActual}
    ...
    declineClient(id){
        let res = {"clientDecl": undefined, "turno": undefined, "newActualClient": undefined, "silla": this.silla}
        ...
        // si el cliente declinado es el turno actual
        {
            ...
            if(newActualClient){
                ...
                res.clientDecl = cliente;
                res.turno = this.turnoNow;
                res.newActualClient = newActualClient;
    ...
    desDecline(id) {
        let res = {"cliente": undefined, "is_actual": undefined}
    ...
    getActual(){
        return ClientDB.findOne({nturno: this.turnoNow, silla: this.silla})
    }
} 
...
class GnClientRepository{
    ...
    send_listo(plqs){
        ...
        for(let plq of plqs){
            ...
            if(indexFirstWait != -1){
                clientes = clientes.slice(0, indexFirstWait);
            }
            clientes_listos = clientes_listos.concat(clientes)
        }

        return clientes_listos;
